...
I have highlighted only those aspects of the proof that are relevant to the assumptions on which it depends, rather than its detailed derivation.\footnote{For a rigorous formulation of the proof, see \textcite[82]{shapiro1991foundations}.} The crutial point, in my view, is that the set theoretic assumptions made are fairly weak: the most demanding conditions are perhaps accepting the power set of the natural numbers and a fairly weak version of replacement, which tends to happen ubiquitously in mathematical practice:
