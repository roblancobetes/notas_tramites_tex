...
I first reconstruct Feferman's conceptual structuralism and its relation to CH. I then argue that the strongest version of Feferman's position does not rest primarily on metamathematical considerations, but on the role different structures play within mathematical and scientific practice...
...
In my view, Feferman is correct in claiming that determinacy in these cases is ultimately accepted as a matter of faith, both in the categoricity of arithmetic and in that of set theory. Nevertheless, I maintain that the two cases differ substantially in mathematical practice. Mathematicians typically do not reason in a purely syntactic fashion, but rather \textit{within} an intended structure. Structures incorporating $\mathbb{N}$ and $\mathcal{P}(\mathbb{N}) \simeq \mathbb{R}$, within which the categoricity theorem for $\mathrm{PA}_2$ can be proved, are generally immediately accessible and appear ubiquitously and consistently across different areas of mathematical practice. By contrast, the full set-theoretic hierarchy rarely appears outside foundational contexts, except perhaps as an intuitive representation of the axioms of set theory themselves. Needless to say, the assumptions underlying the categoricity theorem for arithmetic are substantially weaker than those required in the corresponding case of set theory, as \textcite[141]{Ferreiros2023ConStructuralism} points out:
...
The issue of non-standard models appearing is parallel to the one of categoricity, since they are non-categorical theories which admit non-standard models.

Non standard elements. Order type of the non-standard model:

$$\mathbb{N}+\mu \cdot \mathbb{Z}$$

Lightstone&Robinson 1975 used non-standard reals to proof results on the ordinary reals 

What is the main use of the standard model? It inspired axioms.
...
Note how Koellner correctly identifies methods for recognizing a non-standard model of set theory but fails to address how one would positively identify a standard one. 

One of the central results underlying forcing is the generic filter existence lemma; see, for example, Lemma IV.2.3 in \textcite[246]{Kunen2011}. This result presupposes that the ground model $M \models ZFC$ is countable from the metatheoretical perspective (typically also transitive). Consequently, in many canonical applications of forcing—including Cohen’s original construction—the models that play the central operational role are countable transitive models. This reveals an interesting tension concerning the notion of a “standard model” in set theory. In forcing practice, the structures that mathematicians most directly manipulate are not fully realized platonic universes, but countable transitive models and their forcing extensions. This does not show that countable transitive models are the intended models of set theory, but it does suggest that the practical role of “standardness” in set theory differs significantly from the case of arithmetic, where the standard model is usually regarded as epistemologically privileged from the outset.

This highlights a fundamental disanalogy between arithmetic and set theory. In arithmetic, the standard model, $\mathbb{N}$, is epistemologically privileged. We have a direct pre-theoretical intuition of this structure—it is the very one we use to learn counting. The complete theory of this model serves as a clear (although not recursively computable) benchmark for truth in arithmetic. 
...
\textbf{Tendency im mathematical practice}
