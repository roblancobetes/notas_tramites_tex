...
In these passages, Feferman suggests that merely following whatever mathematical practitioners happen to do at a given historical moment may lead to philosophically fragile accounts of mathematics. At the same time, he is clearly not claiming that philosophy of mathematics should ignore mathematical practice altogether.

\textbf{What does in mathematical practice ensure determinacy?}

\textbf{Commitment to structures vs. commitment to axioms}

\textbf{How does a mathematician imply that they see a structure as determinate? Here are some reasons:}

\begin{enumerate}
  \item Modes of access in practice: Our practical interaction with natural numbers is iterative, algorithmic, sequential. But mathematical practice almost never directly manipulates $P(\mathbb{R})$.
  \item Degree of framework dependence: Arithmetic often functions as a fixed background across foundational programs. Higher set theory often changes together with the surrounding foundational assumptions.
  \item Explanatory indispensability vs. optional enrichment: The natural numbers seem unavoidable in ordinary mathematics. By contrast, large portions of higher set theory function more like optional enrichments of mathematics. CH is a paradigmatic example: vast areas of mathematics proceed unaffected regardless of its truth value.
  \item Convergence of representations: The natural numbers appear monomorphic partly because an enormous variety of mathematical practices converge on the same structure, By contrast, the set-theoretic hierarchy admits radically different extensions and interpretations, such as forcing extensions, inner models, large cardinal hierarchies, determinacy models, multiverse conceptions.
\end{enumerate}

\textbf{Take into account that many is already said in Section \ref{standard-models-epistemology}. Arithmetic and set theory invert the order of whether axioms apply to structures or structures to axioms.}
...
Taking all this into account, I think that the notion of applicability, that Feferman mentions but only indirectly, is nevertheless highly promising.  How CH, or abstract set-theoretical conceptions, might precisely fit into such a framework is a complex topic to think about, that goes beyond the limits of this work (see Author, in preparation).
