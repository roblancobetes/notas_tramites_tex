...
In either case it is not a mean to stablish definiteness in logical terms.

Even if higher logics are defined, what we understand as a logical proof is fundamebtally concerned with first order logic. What these results show is that there is no way of stablishing determinacy of a structure logically, in terms of first order axioms. If one adheres to second order logic, then the determinacy assumptions are implicit in the logic itself. 

However, in practice these two results are quite different. Feferman is right in asserting that determinacy in this cases is accepted as a matter of faith. But, in practice, the determinacy of the structures where the categoricity of arithmetic is proven are usually accepted to be determined. And thus the result is understood \textit{within} these structures.
