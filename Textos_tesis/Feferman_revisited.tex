...
Feferman develops the formal system SCS (Semi-Constructive Set Theory), based on an intuitionistic version of the Kripke–Platek (KP) axiomatic system together with several additional principles. Schematically, the system can be represented as:
...
The quasi-categoricity result of $ZFC_2$ can be formalized within $ZFC$, a similar setting that we found in arithmetic. We can formalize in this system the idea of being a model of full second-order set theory by the relation $(M,P) \models ZFC_2$, where $P=P(M)$ (and similarly for the interpretation of higher order variables), and in such a way that the $ZFC$ axioms hold when relativized to $M$, where the replacement schema can now be seen as a single axiom:
...
\begin{theorem}[Zermelo , Shepherdson]
  
\(\kappa\) is inaccessible iff
\[
V_\kappa \models \mathrm{ZFC}^2 .
\]
\end{theorem}

\begin{proof}
All the axioms of ZFC except Replacement are readily seen to hold in \(V_\alpha\) for any limit ordinal \(\alpha > \omega\). To verify Replacement for \(V_\kappa\), suppose that \(x \in V_\kappa\) and \(F\) is any function $x \to V_\kappa$.

Then $|F''x| \leq |x| < \kappa$ and so by (a) $F''x \in V_\kappa$.

For the converse, first note that \(\kappa\) is regular: If not, there would be an \(\alpha < \kappa\) and a function $G : \alpha \to \kappa$ with a range unbounded in \(\kappa\). Then \(G \subseteq V_\kappa\), and so by second-order Replacement $G''\alpha \in V_\kappa$. Hence, $\sup(G''\alpha)=\kappa \in V_\kappa$, a contradiction.

It remains to establish that \(\kappa\) is a strong limit cardinal. Else, there would be a \(\lambda < \kappa\) such that $2^\lambda \geq \kappa$.

Then
\[
\mathcal{P}(\lambda)\in V_\kappa
\]
by the Power Set Axiom, yet there would be a surjection
\[
H : \mathcal{P}(\lambda)\to \kappa .
\]
So again by second-order Replacement,
\[
H''\mathcal{P}(\lambda)=\kappa \in V_\kappa,
\]
a contradiction.
\end{proof}
