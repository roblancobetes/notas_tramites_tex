...
Thus, Lakatos’ approach is able to illuminate the way mathematical entities, definitions, and structures tend to develop. In order to make my point more explicit, I defend that, if $S$ is the collection of structures (perhaps open-ended) that practicioners of mathematics tend to commit to in a given time, then another structure $X$ should be also included in that collection in case it satisfies at least one of the following conditions:
...
\textbf{Simplicity:} $X$ is---\textbf{in a sense to be discused}---the simplest structure satisfying the previous point, for a set given set of solved problems, 

Whether one commits to structures or to axiom systems, one finds an iterative pattern in which new structures and definitions emerge from previously accepted ones. It would therefore be fruitful to examine the structures and patterns present in contemporary mathematical practice—particularly those involved in the definition of CH—through this perspective. However, there is a point on which greater subtlety is required. This iterative approach does not clearly explain which mathematical notions, if any, could arise independently of previously established notions.
