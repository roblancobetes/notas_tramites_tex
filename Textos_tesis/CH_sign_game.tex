...
Arguments that Shapiro gives related to mathematical practice are also problematic. For example he states (\citeyear{shapiro1991foundations}, p. 99) that important tool in mathematical practice, which is the concept of a minimal closure, cannot be formalized without appealing to second order logic: ``There is a straightforward argument, relying on the compactness theorem, that no collection of first-order formulas can successfully characterize any non-trivial minimal closure. This is evidence that first-order languages are inadequate.'' However, applying this argument to mathematical practice is not that straightforward, since in general practicioners tend to use minimal closures that are defined within a previously given structure. 

Take, for instance, the algebraic closure of $\mathbb{Q}$, which is the most paradigmatic example. This set contains all the solutions to polynomical equations with coeficients in $\mathbb{Q}$, which can be written as $\{ x \in \mathbb{C}: \exists n \in \mathbb{N}, q_0, ..., q_n \neq 0 \in \mathbb{Q},\forall x, q_0 + ... + q_n x^n = 0 \}$. This is a definition with parameters over $\mathbb{Q}$ in the structure $\mathbb{C}$, using the language of first order theory of rings plus a really weak version of first order set theory. More generally, in a given structure, most closures are definable by formulas with parameters or describable as the intersection of definable sets. 

In many cases, this structures are grasped prior to the axiom systems that characterize them, and that it is only later on that we can give axioms as true fundamental statements about them---this is precisely which fits more to \textit{ante rem} structuralism, more than appealing to second order logic and categoricity. These axioms could perhaps be insufficient to completely characterize the structure. In this case, I would say that mathematical practicioners are \textit{commiting to a mathematical structure}.

However, sometimes mathematicians (specially since the advent of the formalist approach) work the other way around, and start working on an axiom system before they can even think of a clear structure satifying them. This was clear in the case of set theory, which I threat in detail in section \ref{set-theory}, and even of category theory. In this case they are \textit{comitting to an axiom system}, which, in case it is a non categorical one, does not imply commiting to a determinate structure. \textbf{relation among them}
