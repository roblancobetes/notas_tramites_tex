...
Moreover, these structures occupy a distinctive position within mathematical practice because of their deep entrenchment in scientific applications and everyday forms of reasoning. Counting, measurement, spatial representation, and quantitative modelling all rely on them in essential ways. In the terminology introduced in Section \ref{quasi-emp-structuralism}, they provide some of the clearest examples of externally applicable structures.

%---------Natural numbers-----------------
$\mathbb{N}$ is the simplest infinite mathematical structure, although it is increbly rich, and in many cases enough for most applications in physics and many areas of mathematics.

It is hard to enumerate all the applications a structure as $\mathbb{N}$ has. \textcite{maclane1986mathematics} defends that the structure of natural numbers is based in practical neccesities. Real-life applicability: time tending to infinity, divisibility of matter, computer programs.

arithmetic operations were invented (or discovered?) because they have all manner of practical uses in financial or scientific 
calculations 10

Such counts are useful in gambling or speculating. Choose three cards in succession from an (ordered) deck of thirteen; what is the chance that they come out in a direct or reverse order? It is the ratio of favorable cases [(123) or (321)] to the total number 6 of cases (of permutations). 12



The possibility of formalyzing, simplifying and solving problems regarding these processes is through the standard model.

Non-standard models: $\omega+ \mathbb{Z} \eta$ 

A non-standard element is an element $a\in M$ such that $0_M, 1_M, 2_M,... < a$

A simple idea, a biyection between the future days in the calendar (the standard model fits but non-standard do not: there are not non-standard days in the calendar).

 Determinateness of this structure: non standard models of $\mathbb{N}$ in practice.  \dots Non-standard models of $PA$ do not represent the notion of counting and tending to infinity as $(\mathbb{N},0,S)$ does. Similarly, non standard models of the axioms of the real line do not parallel our intuition of space as $\mathbb{R}$ does. These structures are priviledged by practice: it is practice, and not logical reasoning, that tells mathematicians to choose them instead of others.

Categoricity arguments around $\mathrm{PA}$. \textcite{Ferreiros2023ConStructuralism} says it is ``easier'' to proof the categoricity of $\mathrm{PA}$. $\mathrm{PA}$ standard model is formalizable within $\mathrm{ZFC}$?

Dedekind's proof is a fairly accepted theorem in practice. It seems that I am taking the position of \textcite{shapiro1991foundations}regarding second order logic, which I was hesitant about in Section 2. Instead, I am just stating that they could be applied, restrictively, much more suitably in the case of arithmetic: it is much easier to argue that second order logic better captures the informal language of mathematics in the case of arithmetic, as in Dedekind's categoricity argument, than in the one of set theory.

This chapter will explore the origins and development of the system of real numbers. These numbers form the most central structure in all of Mathematics. Like other structures it arises from more primitive human activities-in this case, from the measurement and comparison of magnitudes of various kinds. 93

Mathematicians commit to the natural numbers+ there is no recursively enumerable set of axioms that determines this set= there can be cases of committing to a structure that cannot be reduced to a set of axioms.
...
Recall that the standard model of arithmetic can be formalized using the ZF axiom system.
...
Maximal forcing axioms. Woodind's $\Omega$-logic. 
