...
The central claim of this paper is that the determinacy of a mathematical statement depends on whether the structures in which it is evaluated have been adopted in mathematical practice as determinate structures rather than merely as open-ended axiom systems. While arithmetic and analysis satisfy this condition, contemporary set theory largely does not. Consequently, there is currently insufficient reason to regard CH as possessing a uniquely determined truth value.
...
I would argue, however, that this success is primarily linguistic and conceptual rather than foundational. The usefulness of set-theoretic language does not by itself require the adoption of a uniquely determined cumulative hierarchy, nor even to the full strength of the axioms of ZFC. 

For example, principles such as the Axiom of Choice are frequently employed in mathematical practice, but it is often unclear whether their full strength is required, as opposed to weaker principles such as Dependent Choice.

A similar observation applies to the Power Set Axiom. At first sight, it seems built into the very definition of a topological space, since a topology on a set is defined as a collection of subsets of that set. Yet the working topologist typically reasons schematically about a particular family of subsets satisfying certain closure conditions, rather than about a completed totality of all subsets. Theorems are proved about the given topological structure itself. Such schematic reasoning differs significantly from a commitment to an independently existing power set in the full set-theoretic sense.

It is true, for instance, that the definition of the discrete topology, where the topology consists of the entire power set, appears to invoke the full Power Set Axiom. It is important to distinguish, however, between two different situations. On the one hand, the corresponding topological space may be manipulated directly, which is closer to the notion of adopting a mathematical structure introduced in Section \ref{structuralims-ontology-epistemology}. On the other hand, the existence of such a topology may merely be presupposed for theoretical purposes, for example in the construction of counterexamples or in the formulation of general results about topological spaces. In the former case, the structures actually used in practice are very often countable and explicitly given, whereas the latter involves only a schematic appeal to a broader set-theoretic framework, which can be carried through regardless of the adoption of a priviledged set-theoretical hierarchy.

%---Reason 3: metamathematics-------------
I now turn to point \ref{metamathematics} from the list above. Axiomatic set theory often functions in a metamathematical role, providing results about the proving power of specific axiom systems for formalized mathematical languages. Indeed, the connection between mathematical provability and formal derivability from the axioms of set theory is so close in contemporary practice that \textcite[373]{maclane1986mathematics} proposes the following standard of rigor:
...
\section{How CH and finite-order arithmetic fit in quasi-empirical structuralism}
...
\small
\begin{quote}
Mathematics belongs to man, not to God. We are not interested in properties of the positive integers that have no descriptive meaning for finite man (dots). If God has mathematics of its own that has to be done, let him do it himself. \parencite[p. 2]{Bishop1967}
\end{quote}
\end{adjustwidth}
